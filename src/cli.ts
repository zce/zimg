...
cli
  ...
  .option('-p, --pattern <pattern>', 'Glob pattern to match files, default to `**/*.{jpg,jpeg,png}`')
  .option('-n, --filename <filename>', 'Filename of optimized image, default to `[name].[size]`')
  ...
  .option('-f, --force', 'Overwrite optimized images, default to `false`')
